...
tic
Href_t = mod_hyp_solver_q(100,2^10,time);
toc
cnt = 1;
for i = 7:0.5:9
    tic
...
tic
steg = 1024;
Href = mod_hyp_solver_q(steg,5000,time);
toc
cnt = 1;
for i = 6:9
    tic
    M = round(2^i);
    h1(cnt) = 40/(M+2);
    H = mod_hyp_solver_q(M,5000,time);
    x = round(steg/2^i);
    tel = 1;
    for j = 1:x:steg
...
fprintf('Får tidsteg av orden %1.2f \n', ordent(1));
fprintf('Får romsteg av orden %1.2f \n', ordenx(1));
